...
function getAuthUsername() {
    if (typeof window.getAuthUsername === 'function') {
        return window.getAuthUsername();
    }
...
async function checkPackstationCompatibility() {
    ...
    // --- AUTHENTIFIZIERUNG AUS LOGIN HOLEN ---
    const authHeader = getAuthHeader();
    
    if (!authHeader) {
        showError(resultDiv, 'Keine Authentifizierung gefunden. Bitte neu anmelden.');
        return;
    }
    ...
    try {
        ...
        console.log('🔐 Auth Header gesetzt (Base64)');
    }
    ...
    if (environment === 'stg') {
        targetUrl = 'https://depst-mara-stg1-decisionhub.pegacloud.net/prweb/api/HASCI/03/notificationLocations';
        ...
        targetUrl = 'https://depst-mara-prod1-decisionhub.pegacloud.net/prweb/api/HASCI/03/notificationLocations';
...
function showFinalResult(resultDiv, data) {
    ...
    if (results && results.length > 0) {
        ...
        if (data && data.ContainerList && data.ContainerList.length > 0) {
            html += `<p style="color: #856404;font-size:12px;">Message: ${data.ContainerList[0].Message || 'Keine weiteren Details'}</p>`;
        }
        html += `
            <details>
...
// --- 13. Lokales Ergebnis (Fallback wenn API nicht erreichbar) ---
function showLocalResult(resultDiv, compatible, incompatible, errorMessage) {
    let html = '<strong>📊 Packstation-Kompatibilität (Lokale Prüfung)</strong><div class="details">';
